function y=slopefnpdf(x,path,name,type,number,time)
    assert( ismatrix(x) );
    ...
    if(size(slopefnh,1)==0)
        slopefnh=[0];
    end
    if(size(slopefnv,1)==0)
        slopefnv=[0];
    end
    ...
    y=[y;f;xi];
    save(".\"+name+"\"+path+number+"\EOG"+name+type+number+"slopefnpdf"+time+".mat",'y');
end