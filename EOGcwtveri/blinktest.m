...
blinkstart=[];
curpeak=[0 0];
for j=1:size(sacpeakv,1)%each blink is a negative peak following a positive one
    if(sacpeakv(j,1)<0)
        curpeak(1,1)=sacpeakv(j,1);
        curpeak(1,2)=sacpeakv(j,2);
    elseif(sacpeakv(j,1)>0)
        if(curpeak(1,1)<0&&(sacpeakv(j,2)-curpeak(1,2))<100)%following a positive peak, and within 500ms
